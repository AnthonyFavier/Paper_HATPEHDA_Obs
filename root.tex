...
\replaced[comment="I2"]{Naturally, during such task, you would want a pleasant and flexible cooperation with the robot. This involves behaviors like not being bothered too much by the robot or being able to delegate tasks to it, even if it's suboptimal.}
...
\replaced[comment="I3"]{Human-aware task planning has been a topic of interest for many works which proposed and refined a framework called HATP (Human-
Aware Task Planner)}
...
However, a more elaborate planning architecture \added{, inspired by HATP, } was recently proposed~\cite{BuisanA21,buisan:hal-03684211}.
The work introduced the \textit{first} architecture of its type, \replaced{and discusses}{discussing} the abilities and a broad spectrum of the framework.
...
The architecture comprises a \textit{dual}-HTN\replaced[comment="I4"]{ architecture}{s} (Hierarchical Task Network\deleted{s}~\cite{naubooks0014222}), which is a joint task specification model: 
% One HTN is for the human, and the other is for the robot. 
Considers a domain modeler available to describe dual-HTN\deleted{s} specifications for the collaborative planning. 
...
A brief working description of HATP/EHDA is as follows. 
The structure manipulated by the solver is \textit{agent}~\cite{thesisBuisan21,buisan:hal-03684211}.
\replaced[comment="R1"]{Two \textit{agent} structures are used, one for the \textit{human} and one for the \textit{robot}, each includes a model of the corresponding agent.}{More precisely, we also said earlier that a two-agent model represented: the \textit{human} and the \textit{robot}. }
...
\replaced[comment="R2"]{A major assumption of the framework is that agents acts alternatively and one at a time. However, the framework}{Without loss of generality the framework assumes a single agent \textit{decides} to act at a time and, also ``which action to execute?'', e.g., add salt, cook, delay, etc., and similarly, other agents follow. In this round-robin fashion (to include a broad spectrum of problems), it}
uses specific actions to synchronize agents' plans. {\em IDLE} is inserted into its plan when an agent's task network is empty, and {\em WAIT} when it does not have regular applicable actions.
...
{It is a solution for $\mathcal{P}_{hr}$, represented as a tree or a graph, i.e., $G=(V,E)$. Each vertex ($v \in V$) represents \replaced[comment="R3"]{the two \textit{agent} structures (\textit{robot} and \textit{human})}{the robot's belief state}, starting from the root node $s_0^r$. Each edge ($e \in E$) represents a primitive action that is either a robot's action $o^{r}$ or a human's action $o^{h}$. $G$ gets branched on the possible choices ($o^{h}_1$, $o^{h}_2$, ..., $o^{h}_m$). 
...
In practice, we often build relations between \replaced[comment="R4"]{entities in the environment and estimate the knowledge and capabilities of agents around us to have a natural collaboration with them.}{objects and agents or objects and their environment to have natural human-robot collaboration, the way they have been used to gather the knowledge and capabilities of people around us.}
...
\begin{definition}
\textbf{(Places.)} Represented as $\mathit{Places}$, \added[comment="E1"]{it} captures a group of constant symbols such that each member individually captures a pre-specified area in an environment declared by the domain modeler.  
\end{definition}
...
\begin{definition}
    \textbf{(Place Specific State Attribute Function.)} A \deleted[comment="E2"]{{\em grounded} state-variable function {\em aka} a} state attribute of a given state is explicitly associated with a place
...
The state variable observability function maps each state attribute \deleted[comment="E2"]{{\em aka} a grounded \textit{state-variable function}} 
...
\begin{itemize}
    ...
    It ``decide\added[comment="P1"]{s}'' the \textit{essential} changes in the human's belief at this stage to be made such that the relevance of their updated belief (which could still be diverging) becomes ineffective. 
    ...
    The subroutine used to decide which \deleted[comment="P2"]{all} communication actions will appear in the robot's executable plan; is described in the following steps:
    \begin{enumerate}
        \item 
...
Focus on the following two attributes: For a given state, $s_i \in \mathcal{S}$, $f_{\textit{SaltInPot}}^{s_i} \in \{\textit{true, false}\}$ and $f_{\textit{stove}}^{s_i} \in \{\textit{on, off}\}$ such that only \replaced[comment="EE1"]{$f_{\textit{SaltInPot}}^{s_i}$}{$f_{\textit{stove}}^{s_i}$} is \textit{inferrable}, all others belong to $\observable$.
...
E.g., Fig.~\ref{fig:scenarios}(B) showing human knowing \textit{SaltInPot}, is ambiguous in reality. We describe a far more realistic way to estimate the evolution of human belief during planning, using the situation assessment discussed in \textit{Qualitative Analysis}. Explicit reasoning on the human mental state detects and prevents ambiguous situations with communication while also prohibiting the robot from being \replaced[comment="D1"]{too verbose}{mouthy}, as shown in \textit{Quantitative Studies}. 
...
And hence such issues do not affect the solver's \textit{completeness} as far as the situation assessment is concerned. Moreover, our solver handles them as a \textit{relevant} divergence to be aligned. \replaced[comment="D2"]{Thus, the human is just communicated with correct updates.}{Followed by the human is communicated with correct updates.}
