...
% \textcolor{red}{*** We just repeat what we say at the end of def.4 and def.5 ***}
...
% \textcolor{red}{TO BE REMOVE OR MOVED TO "COM ACTION MODELLING" *The robot knows the truth, so technically, it is effective only for human beliefs.
...
(1) For $\varphi_i$, 
% its belief state satisfies the fact being communicated, i.e.,
$f_{\textit{svs}}(g_{(1)l},g_{(2)m},...,g_{(k)n},B_{\varphi_i}^s) = q$, and (2) for $\varphi_j$,
% 's belief state does not satisfy it, i.e., 
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        % If $ca_{\varphi_r, \varphi_h}(...)$ is applied, generates a new belief state following state transition rules, i.e., using $B_{\varphi_h}^{s_i,1} = \gamma(B_{\varphi_h}^{s_i}, ca_{\varphi_r, \varphi_h}(...))$.
        % It continues until the first update generating a human belief s.t. its remaining divergence is ineffective. All actions used from the root until the current belief state are \textit{retrieved}.
...
% Note that the agents could begin with different beliefs.
...
% All other state attributes are \textit{observable}.   
