...
Considering Theory of Mind (ToM) when executing a human-robot shared plan enables a robot to adapt its behavior online. Recent work proposes to replan online while tracking the progress of the task from its own and humans' perspectives. 
Consequently, if it finds a new shared plan, it communicates the humans' part of the plan along with other essential task details and starts to execute its own part. 
In this scenario, ToM is required to handle uncertainties caused mainly due to humans' unplanned initiatives, like their temporary absence or inattention, which hinder humans from entailing knowledge about the task progress. However, plan sharing can be costly and committing every time to a new plan is restrictive to humans, so the contemporary human-aware task planners lift the requirement of shared planning such that they plan for only the robot's  behaviour while considering humans uncontrollable, it anticipates their decisions and actions. Such estimation leads to implicitly coordinated human-robot plans. 
Building on an existing approach, we describe a new planning system that uses ToM to maintain human's-robot's individual mental models, while estimates humans' future initiatives to better maintain their mental model at planning time. 
In particular, we formalize a version of situation assessment which is used for robust task planning. Doing so produces better robot's plans and encourages to communicate with the human only minimally. This concept is implemented, and some qualitative and experimental results are discussed to show \textit{its potential}. 
\end{abstract}
...
\textbf{5} Sandra's work on ToM, add examples where that approach is not relevant   
