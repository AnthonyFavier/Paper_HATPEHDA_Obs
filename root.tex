...
% COMMENT FOLLOWING LINE TO SEE FINAL VERSION 
% \def\state{DRAFT}
...
\begin{abstract}
We consider the human-aware task planning problem where human-robot teams are given a shared task with a known objective to achieve. Recent approaches tackle it by modeling it as a team of independent, rational agents, where the robot plans for both the human's and robot's (shared) tasks. Moreover, the robot knows that humans cannot be administered like artificial agents, so it emulates and predicts the human's decisions, actions, and reactions. 
We describe a novel approach to solve such problems, which models and uses {\em execution-time observability conventions}. Abstractly, this modeling is based on situation assessment, which helps our approach capture the evolution of individual agents' beliefs. Moreover, our method deals with belief divergences that arise in practice by anticipating the evolution of agents' beliefs. It decides {\em if} and {\em when} belief alignment is needed and achieves it with communication. These changes improve the solver's performance: (a) communication is effectively used, and (b) robust for more realistic and challenging problems.
\end{abstract}
...
% \deleted[comment="I1"]{
%some are activities while some are components/objects
the salt-container, adding salt to the pasta, etc. 
Sometimes, some components and activities needed for pasta preparation might be accessible only to you (the robot) from your (its) current position, while some are accessible to both. 
...
% }
% {Naturally, you would want to work with the robot without being bothered too much; or being lazy to do something, assuming the robot will do it instead, even if it takes longer to achieve the task.}
...
%% shashank: NEED TO RESTRUCTURE THIS PARA
...
% } {Human task modeling and human-aware task planning to build a framework focusing on human-robot collaboration and (partially) handling the issues of our interests are discussed}
% \cite{alami2006toward,montreuil2007planning,alili2009planning,alili2009task,lallement2014hatp,de2015hatp,lallement2018hatp}.
% \note{7 works on HATP are cited here.. We should probably only cite 1 or 2.. Then cite more in related work.}
A more elaborate planning architecture, inspired by HATP although extends HTNs differently, is recently proposed~\cite{BuisanA21,buisan:hal-03684211}.
...
The new planning scheme raises several interesting, non-trivial questions to be investigated, and their principled will improve the proposed HATP/EHDA framework toward task-level autonomy for human-robot interaction or human-robot collaboration, to be precise. 
% that we intend to investigate incrementally.

HATP/EHDA comprises a \textit{dual}-HTN framework and can be seen as a joint hierarchical task specification model. 
(We assume a domain modeler is available to describe such hierarchical joint-task specifications.) The joint model describes agents' capabilities, initial beliefs, shared tasks, world dynamics, understanding of common ground, etc. Moreover, the modeler implicitly captures hypothetical variables to represent the human mood, intentions, etc., that are non-trivial to manage explicitly but ``affect'' the human behavior. 
...
% Note that HATP/EHDA  extends 
HATP allows only one search thread for a shared plan, resulting into two coordinated plan streams for the robot and human team. 
However, HATP/EHDA proposed a two-threaded search process, generating a policy tree comprising agents' actions. 
The human agent is \textit{uncontrollable}, so it is not trivial to determine their exact ``next'' feasible action relevant for the overall joint task. It captures the impact of their mental state, mood, intention, etc.  
% Although a human is assumed to be a collaborative and rational agent, jointly working towards a common goal, their next move towards achieving the task, cannot be decided beforehand.
Also, it is important to note that both the robot's and human's HTNs are with the robot. At each stage, it must plan its joint actions following the specifications while predicting, managing, and emulating human decisions, goals, beliefs, etc.
...
In reality, a robot can act that influences human's belief differently under different scenarios. For example, adding salt to the pasta or switching on the furnace (in the kitchen) would impact the human's belief state differently depending on whether the human is ``co-present'' in the kitchen at the time of action execution.   
...
% }{More precisely, we also said earlier that a two-agent model represented: the \textit{human} and the \textit{robot}. }
...
% \replaced[comment="R2"]{
A simplifying assumption is that the agents \textit{act alternatively} and only one is \textit{allowed} to act at a time, i.e., ``which action to execute next?'' E.g., add salt, cook, wait, etc. Implies the framework would face problems dealing with real concurrent actions (CAs)~\cite{CrosbyJR14}, especially when they are \textit{interacting}~\cite{ShekharB20}. 
Therefore, the framework is sound and complete only for   the problem specifications where action concurrency is carefully handled. Later, one can find a sound \textit{parallelization} of agent's plans after post-processing, e.g., managing the causal links and synchronizing agents' action orderings), the solution generated by the deliberation process. 
...
% We do not add any more information to their joint task specification during plan elaboration. 
...
{It is a solution for $\mathcal{P}_{hr}$, represented as a tree or a graph, i.e., $G=(V,E)$. Each vertex ($v \in V$) represents the ground truth 
% \replaced[comment="R3"]{the two \textit{agent} structures (\textit{robot} and \textit{human})}{the robot's belief state}, 
starting from the root node,  i.e., $s_0^r$. 
Each edge ($e \in E$) represents a primitive action that is either a robot's action $o^{r}$ or a human's action $o^{h}$. $G$ gets branched on the possible choices ($o^{h}_1$, $o^{h}_2$, ..., $o^{h}_m$). 
...
In this work, we assume that the robot is always aware of the world state, while the human agent may hold a belief that does not exist in the real world. Moreover, both agents are always certain about their individual beliefs unless they learn otherwise. (In this case, if needed, the human's belief gets updated or aligned with the world state). 
...
% }{objects and agents or objects and their environment to have natural human-robot collaboration, the way they have been used to gather the knowledge and capabilities of people around us.}
...
$\varphi_r$ ($\varphi_h$). (While both these perspectives are managed by the robot in our setting.) 
...
\begin{definition}
    ...
    % \deleted[comment="E2"]{
    {\em grounded} 
    % state-variable function {\em aka} a} 
...
% \deleted[comment="E2"]{{\em aka} a grounded \textit{state-variable function}} 
...
But, without seeing some sugar added, he cannot get `there is sugar' in it. Hence, we say the \textit{SugarInCup} attribute is \textit{inferrable}, 
...
% (It is also the running example.) 
...
\begin{itemize}
    ...
    \item \textbf{Scenario~(B):} Human leaves the kitchen. Practically, she is unaware of the changes achieved in the environment by the robot's actions: {\em turn-on} \& {\em add-salt}. The old solver generated almost the same plan as in {\em Sce.~(A)}, and the human knew that the stove was {\sc on}, and salt was added.
...
And hence such issues do not affect the solver's \textit{completeness} as far as the situation assessment is concerned. Moreover, our solver handles them as a relevant divergence to be aligned. 
% \replaced[comment="D2"]{
Thus, the human is just communicated with correct updates.
% }{Followed by the human is communicated with correct updates.}
