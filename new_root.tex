...
\begin{abstract}
% 
Building on earlier work on Human-Aware Planning, we describe a new approach that:
...
% To improve this solver, we describe a new 
...
% They improve the planner's overall performance: (a) communication is effectively used, and (b) robust for more realistic and challenging problems.
\end{abstract}
...
Anticipating human decisions based on an accurate estimation of their mental state opens the possibility for creating circumstances to promote actions to be performed by them, achieves the joint task or prevents humans from making errors based on their not up to date or inaccurate beliefs.
...
A more elaborated architecture, the ``first'' of its type, called HATP/EHDA, extends existing HATP and better emulates human decisions and actions was recently proposed and showed an improved performance~\cite{BuisanA21,buisan:hal-03684211}.
This new scheme raises interesting, non-trivial questions. Principled answers to these questions will improve the proposed HATP/EHDA framework toward task-level autonomy for human-robot interaction or human-robot collaboration, to be precise. 
...
Such models describe agents'  capabilities, initial beliefs, shared tasks, world dynamics, understanding of common ground, etc. Moreover, the modeler implicitly captures hypothetical variables to represent the human mood, intentions, etc., and are non-trivial to manage. We note that both these models are with the robot while it performs planning. 
...
% But, 
For planning, it proposed a two-threaded search process, generating a joint plan tree comprising agents' actions. 
...
Therefore, for a sound execution, after every robot action: The plan tree branches on all possible legal ``choices'' available to the human.
 
However, 
% being in its early stage of development, 
HATP/EHDA's existing solver makes \textit{simplistic} assumptions during planning like an action executed by an agent would also impact the beliefs of others, i.e., considering the others are always aware of the execution and sharing the same perspective as the former. But, in reality, an agent can execute an action that influences other agents' beliefs differently under different conditions. E.g., \textit{adding} salt or \textit{switching-on} the furnace in the kitchen impacts the human's belief differently depending on whether they are ``co-present'' in the kitchen during execution or assessing the furnace's status now or later.  
So, due to these limitations, the solver often generates plans less robust for real-time execution.     

We describe a new planning approach that uses enhanced reasoning about the human's and robot's individual knowledge/beliefs. To do so, it utilizes a novel paradigm for implementing suitable ``observability'' based conventions understood during plan execution in the human-robot context based on Theory of Mind (ToM)~\cite{devin2016implemented}. 
...
% 
We provide a general description of this formalization, and to support it in the current framework, the existing dual-HTNs formalism is transformed such that the new specifications become less abstract, but enable the new solver to perform enhanced reasoning and to cater to the real-world scenarios that practically are \textit{not possible} to be handled by the old solver, and to support implicit and explicit communication.
...
We evaluate our planner on three \textit{novel} domains, provide qualitative and quantitative analyses, and discuss its theoretical properties.   

(Code, benchmarks (details) and a document for more on the background, related work, and results are provided.)
...
The hierarchical structure of human activities was first exploited by Annett and Duncan~\citeyear{annett1967task}. They said a task could be seen at several abstraction levels until some criterion is satisfied. 
Later, task modeling evolved to introduce system interactions, and its usage became common in user interface designing processes. Most advanced notations include {\sc ConcurTaskTrees}~\cite{paterno2004concurtasktrees} and {\sc hamsters}~\cite{martinie2019analysing}.
Such models are used to evaluate interactive systems while also inspiring this work.
...
% Later, 
Generated plans get shared with the human before the execution~\cite{milliez2016using}.
...
For human-human collaboration (in psychology), spatial reasoning and perspective taking are vital aspects~\cite{flavell1992perspectives,tversky1999speakers}. Through them, a person can mimic the mind of another person to understand their point of view. 
% 
...
\citeauthor{milliez2014framework} (\citeyear{milliez2014framework}) propose visual and spatial perspective taking to find out {\em the referent} indicated by the human. 
...
For elaborated basic terminologies, notation, and definitions related to HTNs, e.g., task network, problem, and solution, the reader is referred to~\cite{naubooks0014222}.  
...
\textbf{(HTN Planning Problem.)} 
The HTN planning
problem is a 3-tuple $\mathcal{P} = (s_0, w_0, D)$ where $s_0$ is the initial belief state (the ground truth), $w_0$ is the initial task network, and $D$ is the HTN planning domain which consists of a set of propositions, tasks and methods.
\end{definition}
...
A brief working description of HATP/EHDA is as follows. The structure manipulated by the solver is \textit{agent}~\cite{thesisBuisan21}. More precisely, we also said earlier that a two-agent model represented: the \textit{human} and the \textit{robot}. 
...
Belief divergence is defined as a situation during planning when there exists an instantiation such that $f_{\textit{svs}_i}:(g_{(1)l},g_{(2)m},...,g_{(k)n},B_{\varphi_r}) = {g_{(k+1)p}}  \neq f_{\textit{svs}_i}:(g_{(1)l},g_{(2)m},...,g_{(k)n},B_{\varphi_h})$.
...
Note that uncertainty in individual's knowledge is not explicitly modeled, and hence each agent is convinced about their belief of the ground truth.
...
% We now define 
Place-based observability criteria is defined abstractly \textit{inspired} by standard execution time observability conventions relevant for the current context~\cite{devin2016implemented}. 
...
Defining it this way captures and maintains the explicit places \textit{linked} with 
``only'' the state attributes of our interests. 
...
Such updates are currently manually handled, carefully.
...
Initially, the cup is far from the human and without sugar. While the human is not attentive (i.e., not \textit{co-present}) to the scenario, the robot adds some sugar to the cup and pushes it towards the human. 
...
% \subsubsection{Delaying Robot Actions for less Divergences}
\subsubsection{Delaying Divergences: A Deliberate Attempt}
...
The main idea here is to exploit our observability-based formalism more such that HATP/EHDA allows the robot to delay its actions in such scenarios. Of course, the trade-off between using communication and \textit{DELAY} actions will affect the final solution plan tree. Currently, we explore this aspect only prototypically. And use it only when the divergence is due to just one inferrable attribute.

% The proposed formalization offers plenty of possibilities to improve the solver. To illustrate its potential, we implemented an additional feature in the planning algorithm.
% A robot action may be \textit{delayed} to make it \textit{observed} by the human and prevent a relevant belief divergence from occurring, and thus, to avoid communication.
% Such feature can be used only if the divergence is due to only one inferrable fact. When a communication action is created, we check if the relevant divergence to align comes from a prior action. If so, the \textit{diverging} action is identified and the plan tree is branched just before this action with a \textit{DELAY} robot action. 
% Then, this new branch is explored by inserting robot \textit{DELAY} actions between estimated human actions until agents are co-present. 
% If co-presence never happens, the branch is deleted. Once co-present, the regular exploration is resumed.

% Thanks to our new model, when possible, a relevant divergence can either be aligned through communication or delaying.
% The choice of alignment type is based on actions' cost estimation which is not our focus, so our costs are currently ad-hoc, as the balance between these two alignments. 
%% But our work is welcoming more accurate cost estimation as part of future work to produce more robust solutions by take into account the applicability and cost of communication actions.
%% Such model can easily be enriched to handle more diverse cases, e.g., interact with blind or deaf people or in noisy place, where the communication is more costly or even impossible.

\section{Formal Properties}
Note that our solver has a similar high-level elaboration process, i.e., its underlying planning mechanism, as the old solver but with a better estimation of the evolution of individual agents' beliefs.  
However, when discussing its soundness and completeness, dependent on the soundness and completeness of the underlying mechanism, we must distinguish between the problem specifications used by the old and new solvers. 
The latter uses a more elaborated encoding formalized earlier. 

In the worst case, the transformed specifications would pick each boolean variable $p \in P$ (all primitive propositions), generate a new set of primitive propositions for every possible combinations of $|\{\observable,\inferrable\}| \times |\mathit{Places}|$. 
...
\end{theorem}
\begin{proof}
(Based on Definition~\ref{def:joint-sol-plan}) We will observe that the joint solution plan generated by our solver is executable:
For a branch, it is guaranteed that it meets all the {\em solution conditions}.
% , e.g., a primitive decomposition to $w_0$. 
Moreover, each precondition for an agent's action in it is either provided by: its own action executed previously, another agent's action occurred in the last time step (this precondition is \textit{inferred}), by another agent via a communication action, or \textit{assessed} by understanding a situation on or before the current time stamp. 
...
Finally, since the human can make any choices during execution, and are unknown upfront, any of its branches, by the above argument, will be executed.
...
\end{theorem}
\begin{proof}
Suppose a problem posed following our new formalism has a solution, say, a joint solution tree, $\tau$ and it is \textit{sound}. 
From it, remove all the communication and situation assessment steps, and say it generates $\tau'$. Now, relax the original problem, considering all the agents being always co-present and making all the propositions \textit{inferrable} everywhere. 
Technically, our solver can  generate $\tau'$ for this relaxed problem when it exhaustively generates the whole search-space (of course it may contain several redundant actions). 
We {\em assure} this by believing in the underlying mechanism and that there is at least one solution ($\tau$) for the original problem. As a result, also it is trivial to visualize that $\tau'$ is always extendable to generate $\tau$ w.r.t. the original problem. Since after the execution of each step, one can employ the situation assessment and action's effect inference subroutines, and check for belief divergence use communication actions if needed. 
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/new_plans.png}
    \caption{
    % Shows the plans obtained in three scenarios: Each scenario presents two plans --- on the left, obtained by the old solver, and on the right, obtained via the proposed solver. The latter depicts more realistic and appropriate belief updates focusing on two attributes.
    % Case~(A): the plans are the same, but the updates in the human belief are more realistic with our approach. 
    % Case~(B): the human has no certainty on {\em SaltInPot}, while ours decides to communicate to remove the ambiguity. 
    % And, Case~(C): the initial belief divergence induces an ``invalid'' plan, but our approach predicts that the human agent will \textit{assess the situation} and update their belief with all the facts without being communicated. (Other minor details are outlined in the figure.)
    Shows the plans obtained in two scenarios: Each scenario presents two plans --- on the left, obtained by the old solver, and on the right, obtained via our solver. The latter depicts more realistic and appropriate belief updates (focusing on two attributes).
    Case~(A): the human has no certainty on {\em SaltInPot}, while ours decides to communicate to remove the ambiguity. 
    And, Case~(B): an initial belief divergence on {\em AtPasta} induces an ``invalid'' plan, but our approach predicts that the human agent will \textit{assess} the pasta location and update their belief, without being communicated.
    % (Important details appear in the box)
    }
    \label{fig:scenarios}
\end{figure}
...
Before we describe {\em three novel domains} used in the experiments and the results to compare the old and new approaches, we first make a ``high-level'' distinction between the two. 
In principle, the old solver can handle a belief divergence only when agents start with distinct beliefs. But, when it is not the case, unlike our solver, it assumes that the agents' beliefs never diverge during planning, hence is no way it can handle a divergence created by an agent in practice. 
To align the beliefs, the old solver achieves it via explicitly influencing (the human's) action model through a cumbersome technique using triggers to update their task networks. Our approach estimates the evolution of belief divergences at planning time and uses explicit communication actions to handle them in practice to generate robust plans. 
...
Pasta can be poured only after salt gets added to the pot and the stove is {\sc on}.
...
Only the human can check and replace lights, and they can start by either of this two tasks.
...
First, qualitative analysis in the first domain to discuss important \textit{subtleties} the old solver overlooks and how ours is aware of them, updates belief, and effectively manages divergences. 
...
We discuss the plans obtained in two scenarios of the first domain as shown in Figure~\ref{fig:scenarios}. Assume that both the agents are in \textit{Kitchen} and the human acts first. Scenario~(A) captures the agents' plans when the pasta is in \textit{Room}. 
Scenario~(B) captures the plans when, in hindsight, the pasta is moved from \textit{Room} to \textit{Kitchen} such that the robot knows it, while the human still has the old belief.
...
\begin{itemize}
    ...
    \item \textbf{Scenario~(A):} The human leaves the kitchen and hence practically is unaware of the changes achieved in the environment by the robot's actions: {\em turn on} and {\em add salt}. 
    ...
    % Moreover, our approach predicts that the human cannot know the \textit{SaltInPot} fact and that, as being a relevant divergence, it needs to be handled via communication. 
...
In all three, we generated 512 different initial states, including 448 (87.5\%) with divergent initial agents' beliefs and 64 states where agents' beliefs are fully aligned initially. 
Overall, 2048 plans were generated.
...
Scenarios beginning with distinct beliefs induce actions often not applicable in another agent's belief state (or the ground truth), evident by the (average) 18.7\% success rate.
...
After a close analysis, we found that $\approx$ 72\% of the plans have divergent human beliefs at least in one branch after the last action (\textit{leaf}) was executed (i.e., \textit{irrelevant} divergences never got communicated). 
...
We formalized execution-time observability conventions based abstractly on situation assessment and action observability criteria, and we employed this formalism to estimate the evolution of the human mental state and capture belief divergences in a principled way. 
A new planning approach is described, utilizes this better estimation of human mental state to plan for more robust and consistent human-robot joint activities such that a relevant belief divergence is tackled by explicitly modeled communication actions. Compared to the old solver, it produces consistent and more robust plans overall. 

Supporting such run-time conventions is crucial for robust planning, while
ignoring them often leads to difficulty in plan execution. 
E.g., Fig.~\ref{fig:scenarios}(B) showing human knowing \textit{SaltInPot}, is ambiguous in reality. 
% We describe a far more realistic way to estimate the evolution of human belief during planning, using the situation assessment discussed in \textit{Qualitative Analysis}. 
Explicit reasoning on the human mental state detects and prevents ambiguous situations with communication while also prohibiting the robot from being mouthy.
% , as the statistics show in \textit{Quantitative Studies}. 
% A plan failure is considered a great risk, and our approach produces consistent and more robust plans.


Our approach does not refute something believed by an agent through situation assessment. 
...
And hence such issues do not affect the solver's completeness as far as the situation assessment is concerned. Moreover, our solver handles them as a \textit{relevant} divergence to be aligned. Followed by the human is communicated with correct updates.
