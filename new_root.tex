...
\begin{abstract}
% 
Building on earlier work about Human-Aware Planning, we describe a new approach that:
...
Benchmarks (details), code, and a document detailing the background, related work, and results are provided.
...
Our solver does not use these actions (for the deliberation process) like (non-) primitive tasks.
...
(\textbf{Solution Plan}.) 
{A sequence of primitive actions $\pi=(o_1,o_2,o_3...,o_k)$ is a solution plan for the HTN planning problem $\mathcal{P}=(s_0,w_0,D)$ iff there exists a primitive decomposition $w_p$ (for $w_0$), and $\pi$ is an instance of it. 
}  
\end{definition}
...
As the robot is always aware of ground truth, technically, it is effective only for the human belief.
...
Second, communicating relevant divergences to the human agent, and is our main focus in this work. However, delaying may not always find a solution, but it could reduce communication requirements. (Our pilot development and results are in \textit{Appendix~C}.)

Assessing the impact of human actions (based on their wrong $\mathit{B}_{\varphi_h}$) of different effects, with their overall positive and detrimental impacts on achieving a joint task can be interesting.
...
% \begin{itemize}
    ...
    We note that there exists at least one attribute-value pair in receiver's belief causing adverse effects. 
    The approach, explained in the next \textit{three steps}, looks for minimal number of such attribute-value pairs to be communicated so that
    % its modified belief (updated, but could still be diverging) becomes \textit{irrelevant}. 
    the divergence in the updated beliefs is made \textit{irrelevant}, or even eliminated.
...
% Analysis i
In the first domain to discuss \textit{subtleties} the old solver overlooks and how ours is aware of them, updates beliefs, and effectively manages divergences. 
