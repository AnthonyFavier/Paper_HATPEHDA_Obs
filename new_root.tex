...
Therefore, for a sound execution, after every robot action, the policy tree gets branched on all ``legal'' possible \text{choices} available to the human.
...
After that, we present the results and show examples where handling the belief divergences is effective and how HATP-EHDA is ineffective under those scenarios. We compare the overall effectiveness of our approach and show that it uses communication when needed (avoids verbosity) in the process. 
...
Suppose $s \in \mathcal{S}$ is the real state with the ground truth. As per our assumptions, the belief state of the robot w.r.t. the real-world is always the real world state, i.e., $B_{\varphi_r}^s = s$, and the human belief w.r.t. $s$ is $B_{\varphi_h}^s$ --- that is generated when the robot takes the human's perspective. Each state variable function instantiation with respect to a belief state, $B_{\varphi_r}^s$ ($B_{\varphi_h}^s$), represents the truth value of that state attribute with the \textit{perspective} of that particular agent: $\varphi_r$ ($\varphi_h$). (Both perspectives are managed by the robot in our setting.) 
...
We now introduce state-variable attributes with standard observability conventions.
...
\begin{definition}
    \textbf{(Place Specific State Attribute Function.)} A {\em grounded} state-variable function {\em aka} state attribute in a given state is associated with a place or, in a more general way, it is expressed as, $f_{pssa}: (f_{svs}(?g_1(gr_1),...,?g_k(gr_k),\mathcal{S})) \rightarrow 
    % \mathit{Places' \cup \{un\} \equiv 
    Places$
    \label{def:pssav}
\end{definition}
Here, $\mathit{pssa}$ stands for {\em place-specific state attribute}, and it captures place specific attribute for a given state. 
...
To ``learn'' the exact \textit{status} of an attribute: 
% (w.r.t. a state), 
...
Or, the agent is communicated about the exact value of this attribute explicitly. Note that attribute values belonging to both these classes can be communicated explicitly. 
...
To work it out more formally:

% We introduce two new fun
\begin{definition}
\textbf{(Place Based Observability Function.)} 
% (ref to Obesrvability of fluents section)}
The place based observability function maps each state attribute {\em aka} a grounded \textit{state-variable function} 
to either $\observable$ or $\inferable$; or a general way to express it is, $f_{\textit{pbof}}: (f_{svs}(?g_1(gr_1),...,?g_k(gr_k)
% ,\mathcal{S}
)) \rightarrow 
    \{\inferable, \observable\}$.
\end{definition}
% For now, we assume the observability types are constant and initially defined.
Here, \textit{pbof} represents the \textit{place based observability function}.
...
%TODO
% Fig3 A : show when agents copresent 
% show from what process beliefs are updated

% visually show situation assesmsent (black point after action)

% S w/ how much with com 

% put number of scenario 512

% small cartoon: to show difference between obs and inf
% robot and human, human not looking 
% move object 
% add salt
...
An interesting case to focus on is a scenario close to (C) with a belief divergence on the pasta location. Both agents are in the kitchen and the pasta is in the other room, but the human thinks the pasta is in the kitchen. Without the contribution we estimate that the human will try to grab pasta in the kitchen which will fails in real life. In reality the human will be able to assess that the pasta isn't in the kitchen. 
