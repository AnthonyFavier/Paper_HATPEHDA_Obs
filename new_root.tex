...
Without loss of generality, the framework assumes a single agent decides to act at a time and ``which action to execute?'', e.g., \textit{add salt}, \textit{cook}, \textit{delay}, etc., and similarly, other agents follow. Despite this round-robin approach: For handling its broad horizon, the framework uses specific actions to synchronize agents' joint plans. \textit{IDLE} is concatenated to its plan when an agent's task network is empty. And, \textit{WAIT} is concatenated to its plan when not having an applicable action.
