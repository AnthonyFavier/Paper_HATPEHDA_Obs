...
Naturally, the human would want to work alongside the robot without being bothered too much. Sometimes they are too lazy to perform an action assuming that the robot will do it instead, even if it takes longer to achieve the joint task. 
In the cases that interest us, the human is considered a fully cooperative and rational partner but cannot be controlled like an artificial agent (\textit{fully controllable}). 
If the human has several ways to achieve a goal or to accomplish a task, the robot cannot control which way the human selects. However, it can still decide to act to influence human choices, thus eliciting their future actions. 
...
Human task modeling and human-aware planning to build a framework focusing on human-robot collaboration and (partially) handling the issues of our interests are discussed~\cite{alami2006toward,montreuil2007planning,alili2009planning,alili2009task,lallement2014hatp,de2015hatp,lallement2018hatp}.
However, a more suitable planning architecture than these earlier works; is recently proposed.
The work introduced the \textit{first} planning architecture of its type, discussing the abilities and a broad horizon of such a task planning framework~\cite{BuisanA21,buisan:hal-03684211}.
The architecture is named HATP/EHDA, which is human-aware task planning by emulating human decisions and actions. This research, of course, raises several interesting, non-trivial questions to be investigated in the future. Principled answers to these questions will improve the proposed HATP/EHDA framework toward task-level autonomy for human-robot interaction or human-robot collaboration, to be precise. 
...
Assuming that a domain modeler is available with expertise in robotics, human-human collaboration, and human psychology to describe dual-HTNs specifications for human-robot collaborative planning. 
The joint model describes agents'  capabilities, initial beliefs, shared tasks, world dynamics, understanding of common ground, etc. Moreover, the modeler implicitly describes \textit{hypothetical} variables to capture a human's mood, intentions, etc., and are non-trivial to manage. It is important to note that both these models are with the robot, while the robot plans their joint actions based on the specification models, predicts, and emulates human actions, decisions, and reactions.    
...
While in human-robot interaction (HRI), communicating too much or too little can disturb a task achievement process. 
...
Ideas from psychology got employed over the years in human-robot interaction/collaboration.
\citeauthor{johnson2005perceptual} (\citeyear{johnson2005perceptual}) propose a technique based on visual perspective taking for a robot recognizing an action executed by another robot. 
...
% We build a background on the HTN and HATP/EHDA frameworks, providing only relevant definitions and discussing problem models and planning frameworks to understand this work. Situation assessment is then briefly introduced. 
...
For ease of exposition, consider a team of a single human and a single robot. Two categories: The robot model, $\mathrm{HTN}_{r}$, represents the task specifications for a controllable agent, while $\mathrm{HTN}_{h}$, represents the model for the human partner, who is an uncontrollable one but the planner has its model
...
We are interested in solving the HATP/EHDA problem, $\mathcal{P}_{hr}$, in which both agents start with a joint task network to decompose, which means they have a shared initial network $w_0$ to start with.
...
\begin{figure}
    ...
    % While  
    Human is not attentive when Robot \textit{adds} sugar to the cup before \textit{pushing} it forward. \textit{Assessing} the new situation after the two actions, Human can only ``know'' the cup's new position (\textit{observable}), but not \textit{SugarInCup} (\textit{inferrable}).
    }
    \label{fig:obs_attr}
\end{figure}
...
\begin{figure*}[!ht]
    ...
    % Plans obtained for three different scenarios, with and without our contribution, depicting more realistic belief updates of some relevant facts. The existing planning system systematically updates the agents' beliefs while our approach considers observability to do so. 
    % The figure shows three types of human belief updates: [COM] when being explicitly communicated a fact by the robot, [INF] when attending an action with inferrable effects and [SA] when assessing observable facts in the environment. 
    ...
    % In (A) the plans are the same but the human belief updates are more realistic with our approach. In (B) the human has no certainty that the salt has been added, our approach communicates to remove the ambiguity. In (C), the initial belief divergence induce a non valid plan, but our approach predicts that the human will assess the situation and fix the divergence themselves, without communication.
    }
    \label{fig:scenarios}
\end{figure*}
...
\begin{itemize}
    ...
    However, currently, our reasoning system is myopic. We do not evaluate in a principled way the impact of \textit{different} actions that humans could execute (based on their wrong belief), or of \textit{different} effects, with their overall positive and detrimental effects on achieving the joint task. Therefore, we ``decide'' to align the agent's beliefs using communication actions whenever the belief divergence is relevant.
    ...
    \begin{enumerate}
        ...
        % I.e. $$
        \item For each of these attributes, form a $\textit{comm-action}_i$ as per the described communication action model earlier. We assume that all these new communication actions are equally costly. 
        ...
        We then \textit{retrieve} all the communication actions used from the root until the current belief state.   
    \end{enumerate}
...
Based on this new approach, the robot uses the \textit{required} communication actions after each stage of plan elaboration (keep in mind the HATP/EHDA framework's plan elaboration process). 
...
First, some qualitative results in the cooking pasta domain, mentioning important \textit{subtleties} that the old solver misses out on, while how our approach systematically catches them, updates belief, and effectively manages belief divergences. Next, we discuss quantitative results obtained and compare the old and new solvers in both domains.
...
\begin{itemize}
    \item \textbf{Scenario~(A):} The same plan is generated by the solvers. But, notice that both solvers updated the human's belief differently depending on whether the human is co-present with the robot. E.g., turning on the stove ideally (realistically) should not affect the human belief state if not co-present, and this is not the case with the old solver. It considers agents omniscient -- results in the human instantly knowing everything once achieved. With the new approach, the human returns to the kitchen and assesses that the stove is {\sc on}. And the human belief is updated.
    ...
    \item \textbf{Scenario~(C):} In the updated scenario, using the old approach, the human moves to \textit{Room} to grab the pasta, but in reality, the action \textit{fails} as it is not a legal action w.r.t. the robot's belief or the ground truth. But, the way our planning system works, the human agent assesses the environment and updates their belief state, knowing that the pasta is in the kitchen. Hence no communication is required to align their belief. The rest is self-evident.
\end{itemize}
...
Such deadlocks can occur when the human has a belief divergence, making them wait for the robot's actions that never execute (e.g., the human waits for the robot to add salt but \textit{SaltInPot} already holds for it).
...
As the existing solver does not handle belief divergence in planning, the applicability of actions is never an issue w.r.t. another agent's belief. Therefore, if the \textit{IDL} case occurs, it is understood that the task specification is erroneous. However, we handle belief divergences in planning, captured and supported by our \textit{observability} based tools, employed in our solver, developed to cater to more realistic scenarios.

Our solver always finds legal plans, and on average, \textit{approx.} 62\% of them use \textit{communication}.
We note that the robot doesn't need to communicate compulsively/systematically as the situation assessment helps solve a relevant part of the divergences (87.5\% of the scenarios have divergent beliefs initially).
For the old solver, if no initial belief divergence exists, it always finds a legal plan, considering the agents omniscient. E.g., Figure~\ref{fig:scenarios}(A). However, sometimes, this causes problems during execution.
Most scenarios with initial belief divergence induce actions not applicable in another agent's belief (or the ground truth),
% , obviously the case as the 21.8\% success rate on average.
which is reflected in the 21.8\% success rate on average.
...
\begin{table}
    ...
    {
    \label{tab:q_results}
    Comparison of the two solvers on the two domains. For each domain: for the \textit{old solver}, the success rate (\textit{S}), the ratio of failed plans due to a non-applicable action (\textit{NA}), and the ratio of failed plans due to an inactivity deadlock case (\textit{IDL}), while for \textit{our solver}, the success rate (\textit{S}) and the ratio of plans including a communication action (\textit{Com}).
    }
\end{table}

\section{Discussion}
The run-time observability conventions, especially action observability, are crucial for planning. Ignoring them leads to difficulty in practice, sometimes. 
E.g., Fig.~\ref{fig:scenarios}(B) depicts that human knowledge about \textit{SaltInPot} is ambiguous in reality. This paper describes a far more realistic way to estimate the evolution of human belief during planning, using the situation assessment discussed in \textit{qualitative analysis} of the experiments. Explicit reasoning on the human mental state detects and prevents ambiguous situations with communication actions while also prohibiting the robot from being mouthy, as shown in \textit{quantitative studies}. 
% A plan failure is considered a great risk, and our approach produces consistent and more robust plans.
Compared to the old solver, overall, our approach produces \textit{consistent} and more \textit{robust} plans.
