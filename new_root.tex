...
This work abstractly employs in planning the main ideas of situation assessment based on spatial reasoning and observability during planning. The goal is to predict the situation assessment of a collaborative human agent to have a better estimate of their knowledge and next possible actions, and thus, to produce better plans.
...
Note that we do not model uncertainties in the individual agents' knowledge. So, even if there are belief divergences, the agents are convinced about their individual beliefs in the ground truth.
...
Such changes are possible when an action changes the \textit{place} of effectiveness of a state attribute, e.g., the robot \textit{holding} a cup and \textit{moving} to an adjacent room. Such updates are currently manually handle% to make the planning sound.
...
However, such inference only happens when the human agent ``observes'' it while the robot adding salt to the pasta, i.e., both agents need to be co-present in the kitchen throughout this process; otherwise, the human will miss the \textit{salt-in-pasta} fact. 
...
The agent examines and updates its belief when relevant ground truth is learnedt via situation assessment, performed ``immediately'' after the execution of each action. Hence, technically, the robot performs situation assessment for both the human agent and itself.
...
For an agent to decide make a decision for "if", "when", and "what" to \textit{communicate} to another agent, it first needs to establish a common protocol with that agent even before planning.  
