...
(Code, benchmarks (details), and a document detailing the background, related work, and results are provided.)
...
The hierarchical structure of \textit{human} activities was first exploited by Annett and Duncan~\citeyear{annett1967task}. They said a task could be seen at several abstraction levels until some criterion is satisfied. 
...
Such models are used to evaluate interactive systems and also inspire us to choose hierarchical models.
...
% In psychology, 
For collaborative humans, spatial reasoning and perspective taking are vital aspects~\cite{flavell1992perspectives,tversky1999speakers}. Through them, a person can mimic the mind of others to understand their viewpoint. 
% 
...
Such relations are stored in databases, allowing access to the complete framework, and used for either planning, acting, or both.
...
Following ToM, our approach estimates the evolution of the agents' beliefs and decides ``if'' and ``when'' belief alignment is necessary for planning. 
And, it is achieved via explicit communication actions, answering ``what" to communicate. 
Our solver does not use these actions (for the deliberation process) like (non-) primitive tasks but to minimally align an agent's belief with the ground reality 
...
Following is its brief working description: The structure manipulated by the solver is \textit{agent}~\cite{thesisBuisan21}. More precisely, we also said earlier that a two-agent model represented: the \textit{human} and the \textit{robot}. 
...
One simplifying assumption the framework makes is if the actions of the human and robot at a given time stamp are ``interacting'' then they need to be modeled explicitly, helping \textit{parallelization} to be sound~\cite{CrosbyJR14}. More formally, interacting actions are those whose combined effect differs from the union of their individual effects, e.g.,~\cite{ShekharB20}. 
...
Each branch, from the root to a leaf node, the sequence of primitive actions, say,  $\pi=(o_1^r,o_2^h,o_3^r,...,o_{k-1}^h,o_k^r)$, must must satisfy all the solution conditions. 
% for
...
% of our interests. 
Such explicit mappings suggest that the attribute is effective (\textit{observable}) in its dedicate place.
...
The human and robot agents carry individual, distinct belief states w.r.t. the ground truth (s.t. $\mathit{B}_{\varphi_r}$ is aligned with it), while the two can be either different or aligned. 
...
At a stage, if the human agent, based on $\mathit{B}_{\varphi_h}$, can perform a set of actions that differs from what they could perform w.r.t. to $\mathit{B}_{\varphi_r}$ (or the ground reality), then we say that such divergent beliefs are \textit{relevant} to be \textit{tackled}.
A divergence is also declared as \textit{relevant} if an action has different effects w.r.t. the other agent's belief.

One approach to tackle it could be to trace back to the action that creates this divergence and {\sc delay} it for future, if and when possible. E.g., the robot delays adding salt until the human arrives in the kitchen. 
Second, communicating relevant divergences to the human agent, and is our main focus in this work. However, delaying may not always find a solution, but it could reduce communication requirements. (Our preliminary thoughts and results are in Appendix~C.)

Formally evaluating the impact of different actions that humans could execute (based on wrong $\mathit{B}_{\varphi_h}$), or of different effects, with their overall positive and detrimental effects on achieving the joint task can be interesting.
An approach might be to analyze the history of plan trace(s) or agents' future actions to decide whether their beliefs should get aligned or not, or for that matter, how much to communicate, but it is currently out of the scope of this work. 
...
At a given stage of planning, e.g., the ground truth ($s_i$), we assume that \textit{Updates Workflow} was already called, and then the sender reasoned out and decided to \textit{tackle} the receiver's divergent beliefs. 
...
% \begin{itemize}
    ...
    The new approach decides only the essential changes in the receiver's belief at this stage to be made. 
    As the divergence is relevant, at least one attribute-value pair must exist in its belief, which is causing the adverse effect. So, the approach looks for such attribute-value pairs to be communicated so that the relevance of their updated belief (which could still be diverging) becomes ineffective, and for that following steps are followed. 
...
Moreover, each precondition for an agent's action is achieved in earlier time stamp by either: its own action, 
% another agent's action occurred in the previous time step (this precondition is \textit{inferred}), 
\textit{inferred} while observing another agent's acting, 
% by another agent communicated previously, 
...
\begin{figure}[t!]
    ...
    \caption{
    Plans obtained in two scenarios: Each presents two plans. (\textit{left}) Obtained by the old solver, and (\textit{right}) obtained by our solver. The latter depicts more realistic and appropriate belief updates (focusing on two attributes).
...
Technically, our solver will generate $\tau'$ for this new problem when it exhaustively generates the whole search space (of course, it may contain redundant actions). 
We {\em assure} it based on the underlying mechanism and that there is at least one solution ($\tau$) for the original problem. As a result, it is trivial to visualize that $\tau'$ is always extendable to generate $\tau$ w.r.t. the original problem. Since after the execution of each step, one can employ the situation assessment and action's effect inference subroutines, checking for belief divergences and using communication actions if needed. 
...
Qualitative analysis in the first domain to discuss important subtleties the old solver \textit{overlooks} and how ours is \textit{aware} of them, updates belief, and effectively manages divergences. 
...
Scenario~(B) 
% captures the plans when, 
in hindsight, the pasta is moved from \textit{Room} to \textit{Kitchen} such that the robot knows it, while the human still has the old belief.
...
\begin{table}
    ...
    \begin{tabular}{@{}c|r r r| c@{}}
        ...
        \textit{Car} & 12.5\% & 73.2\% & 26.8\%  & 83.6\%\\
        \hline
        \textbf{Average} & 18.7\% & 77.8\% & 22.2\%  & 69.1\%\\
    \end{tabular}
    \end{adjustbox}
    \caption
    {
...
Such deadlock occurs when the human has a belief divergence and waits for a never-happening robot's action, e.g., waiting for \textit{add salt}, but \textit{SaltInPot} is already achieved.
...
We formalize run-time observability conventions based abstractly on situation assessment and action observability. 
We use this formalism to estimate the evolution of the human mental state and capture belief divergences. 
A new planning approach is described, utilizes the estimation of human mental state to plan for robust and consistent human-robot joint activities such that a relevant belief divergence is tackled by explicitly modeled communication actions. Compared to the old, ours produces consistent and more robust plans. 

First of all, we \textit{put} an effort to make our main contributions to have a ``generic'' description. Hence, they are not limited to only our intended HATP/EHDA framework.
% , which we plan to enhance in this paper.

Also, a principled way to handle those conventions is vital for robust planning, as, otherwise, it can lead to difficulty at run-time. 
E.g., Fig.~\ref{fig:scenarios}(A): the human knowing \textit{SaltInPot} is ambiguous. 
Explicit reasoning on the human mental state detects ambiguous situations and removes them via communication while avoiding the robot from being mouthy.

Our formalism does not refute something believed by an agent through situation assessment. 
E.g., for some $s_i \in \mathcal{S}$, if the human wrongly believes that the pasta is in \textit{Kitchen}. The situation assessment does not help refute this, while the human is in \textit{Kitchen}. 
The reason is that $f_{\textit{pasta-not-in-kitchen}}^{s_i}(...)$ is not modeled explicitly as an attribute. 
And hence such issues do not affect the its completeness. 
% as far as the situation assessment is concerned. 
